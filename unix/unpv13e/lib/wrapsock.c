...
{
    if (bind(fd, sa, salen) < 0) err_sys("bind 错误");
}

void Connect(int fd, const struct sockaddr* sa, socklen_t salen)
{
    if (connect(fd, sa, salen) < 0) err_sys("connect 错误");
}

void Getpeername(int fd, struct sockaddr* sa, socklen_t* salenptr)
{
    if (getpeername(fd, sa, salenptr) < 0) err_sys("getpeername 错误");
}

void Getsockname(int fd, struct sockaddr* sa, socklen_t* salenptr)
{
    if (getsockname(fd, sa, salenptr) < 0) err_sys("getsockname 错误");
}
...
{
    if (inet6_rth_add(rthbuf, addr) < 0) err_quit("inet6_rth_add 错误");
}

void Inet6_rth_reverse(const void* in, void* out)
{
    if (inet6_rth_reverse(in, out) < 0) err_quit("inet6_rth_reverse 错误");
}
...
int Inet6_rth_segments(const void* rthbuf)
{
    ...
    if (ret < 0) err_quit("inet6_rth_segments 错误");
...
struct in6_addr* Inet6_rth_getaddr(const void* rthbuf, int idx)
{
    ...
    if (ret == NULL) err_quit("inet6_rth_getaddr 错误");
...
int Kqueue(void)
{
    ...
    if ((ret = kqueue()) < 0) err_sys("kqueue 错误");
    return ret;
}
...
void Listen(int fd, int backlog)
{
    ...
    if (listen(fd, backlog) < 0) err_sys("listen 错误");
}
/* end Listen */
...
int Poll(struct pollfd* fdarray, unsigned long nfds, int timeout)
{
    ...
    if ((n = poll(fdarray, nfds, timeout)) < 0) err_sys("poll 错误");
...
ssize_t Recv(int fd, void* ptr, size_t nbytes, int flags)
{
    ...
    if ((n = recv(fd, ptr, nbytes, flags)) < 0) err_sys("recv 错误");
    return (n);
}
...
ssize_t Recvmsg(int fd, struct msghdr* msg, int flags)
{
    ...
    if ((n = recvmsg(fd, msg, flags)) < 0) err_sys("recvmsg 错误");
    return (n);
}
...
{
    if (send(fd, ptr, nbytes, flags) != (ssize_t)nbytes) err_sys("send 错误");
}
...
void Sendmsg(int fd, const struct msghdr* msg, int flags)
{
    ...
    if (sendmsg(fd, msg, flags) != nbytes) err_sys("sendmsg 错误");
}
...
{
    if (shutdown(fd, how) < 0) err_sys("shutdown 错误");
}
...
int Socket(int family, int type, int protocol)
{
    ...
    if ((n = socket(family, type, protocol)) < 0) err_sys("socket 错误");
    return (n);
}
/* end Socket */
