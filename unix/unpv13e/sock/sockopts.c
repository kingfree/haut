...
void sockopts(int sockfd, int doall)
{
    ...
    if (debug) {
        ...
        if (option == 0) err_quit("SO_DEBUG not set (%d)", option);

        if (verbose) fprintf(stderr, "SO_DEBUG set\n");
    }
    ...
    if (dontroute) {
        ...
        if (option == 0) err_quit("SO_DONTROUTE not set (%d)", option);

        if (verbose) fprintf(stderr, "SO_DONTROUTE set\n");
    }
    ...
    if (iptos != -1 && doall == 0) {
        ...
        if (option != iptos) err_quit("IP_TOS not set (%d)", option);

        if (verbose) fprintf(stderr, "IP_TOS set to %d\n", iptos);
    }
#endif
    ...
    if (ipttl != -1 && doall == 0) {
        ...
        if (option != ipttl) err_quit("IP_TTL not set (%d)", option);

        if (verbose) fprintf(stderr, "IP_TTL set to %d\n", ipttl);
    }
#endif
    ...
    if (maxseg && udp == 0) {
        ...
        if (setsockopt(sockfd, IPPROTO_TCP, TCP_MAXSEG, &maxseg,
                       sizeof(maxseg)) < 0)
        ...
        if (verbose) fprintf(stderr, "TCP_MAXSEG = %d\n", option);
    }

    if (sroute_cnt > 0) sroute_set(sockfd);
    ...
    if (broadcast) {
        ...
        if (option == 0) err_quit("SO_BROADCAST not set (%d)", option);

        if (verbose) fprintf(stderr, "SO_BROADCAST set\n");
        ...
        if (onesbcast) {
            ...
            if (option == 0) err_quit("IP_ONESBCAST not set (%d)", option);

            if (verbose) fprintf(stderr, "IP_ONESBCAST set\n");
        }
#endif
    }
    ...
    if (joinip[0]) {
        ...
        if (verbose) fprintf(stderr, "IP_ADD_MEMBERSHIP set\n");
    }
#endif
    ...
    if (mcastttl) {
        ...
        if (ttl != mcastttl) err_quit("IP_MULTICAST_TTL not set (%d)", ttl);

        if (verbose) fprintf(stderr, "IP_MULTICAST_TTL set to %d\n", ttl);
    }
#endif
    ...
    if (keepalive && doall && udp == 0) {
        ...
        if (option == 0) err_quit("SO_KEEPALIVE not set (%d)", option);

        if (verbose) fprintf(stderr, "SO_KEEPALIVE set\n");
    }
    ...
    if (nodelay && doall && udp == 0) {
        ...
        if (option == 0) err_quit("TCP_NODELAY not set (%d)", option);

        if (verbose) fprintf(stderr, "TCP_NODELAY set\n");
    }
    ...
    if (recvdstaddr && udp) {
        ...
        if (option == 0) err_quit("IP_RECVDSTADDR not set (%d)", option);

        if (verbose) fprintf(stderr, "IP_RECVDSTADDR set\n");
#else
    ...
    if (sigio) {
        ...
        if (verbose) fprintf(stderr, "FIOASYNC set\n");
#else
