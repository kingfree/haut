...
#define error(e)     \
    ...
    for (sptr = &search[0]; sptr < &search[nsearch]; sptr++) {
        ...
        if (isdigit(sptr->host[0])) {
            ...
            if (inet_pton(AF_INET, sptr->host, &inaddr) == 1) {
                ...
                if (sptr->family != AF_INET) continue; /* ignore */
        ...
        if ((isxdigit(sptr->host[0]) || sptr->host[0] == ':') &&
            ...
            if (inet_pton(AF_INET6, sptr->host, &in6addr) == 1) {
                if (hints.ai_family != AF_UNSPEC && hints.ai_family != AF_INET6)
                    error(EAI_ADDRFAMILY);
                if (sptr->family != AF_INET6) continue; /* ignore */
        ...
        if (hptr == NULL) {
            if (nsearch == 2) continue; /* failure OK if multiple searches */
        ...
        if (hostname != NULL && hostname[0] != '\0' &&
            ...
            if ((canon = strdup(hptr->h_name)) == NULL) error(EAI_MEMORY);
        }
    ...
    if (aihead == NULL) error(EAI_NONAME); /* nothing found */
    /* end ga4 */
