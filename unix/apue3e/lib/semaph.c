...
again:
    ...
    if (semval == 0) {
        ...
        if (semctl(id, 0, SETVAL, semctl_arg) < 0) err_sys("can SETVAL[0]");

        semctl_arg.val = BIGCOUNT;
        if (semctl(id, 1, SETVAL, semctl_arg) < 0) err_sys("can SETVAL[1]");
    }
    ...
    if (semop(id, &op_endcreate[0], 2) < 0) err_sys("can't end create");
...
void sem_rm(int id)
{
    if (semctl(id, 0, IPC_RMID, 0) < 0) err_sys("can't IPC_RMID");
}
